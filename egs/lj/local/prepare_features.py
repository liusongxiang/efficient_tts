...
def preprocess_run(args):
    ...
    # Dirs
    vctk_mfa_dir = f"{args.data_root}/{args.mfa_folder}"
    vctk_wav_dir = f"{args.data_root}/{args.wav_folder}"

    # Get file id list
    fid_list = []
    # fid_list.extend(glob.glob(f"{libritts_mfa_dir}/*.dur.npy"))
    fid_list.extend(glob.glob(f"{vctk_mfa_dir}/*.dur.npy"))
    # fid_list.extend(glob.glob(f"{cmuarctic_mfa_dir}/*.dur.npy"))
    ...
    for lab_fid in fid_list:
        ...
        wave_path = f"{vctk_wav_dir}/{spk_name}/{fid}.wav"
...
def get_parser():
    ...
    parser.add_argument('--data_root', default=None, required=True, help="Data directory.")
    parser.add_argument('--wav_folder', default=None, required=True, help="Wave folder name.")
    parser.add_argument('--mfa_folder', default=None, required=True, help="Montreal Forced-Aligner folder name.")
